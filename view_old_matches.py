...
def get_old_matches(player_name: str):
    ...
    cursor.execute("""
        SELECT game_id, player1_name, player2_name, is_active, created_at
        FROM games
...
def display_old_matches(player_name: str):
    ...
    for match in matches:
        game_id, p1, p2, is_active, created_at = match
        ...
        # Determine result if winner info is not stored (not in schema)
        result = "COMPLETED" if not is_active else "ACTIVE"
