...
1. **Service-to-Service Authentication**
   ...
   - Service API keys can be generated by `build-and-start.sh` or `build-and-start.ps1` scripts (optional)
...
Service API keys can be automatically generated using the build scripts (`build-and-start.sh` for Linux/Mac or `build-and-start.ps1` for Windows). The keys are:
...
**Note**: For basic development with `docker compose up`, services will work without these keys. The scripts are recommended for production deployments requiring enhanced security.
