...
def auto_resolve_round(game, conn):
    ...
    try:
        ...
        game_over, winner, is_tie, tie_breaker_possible = get_game_end_status(p1_deck, p2_deck, new_p1_score, new_p2_score)
        ...
        cursor.execute("""
            ...
            WHERE game_id = %s
        """, (new_p1_score, new_p2_score, not game_over, winner_name, game['game_id']))
...
def check_tie_breaker_status(game_id):
    ...
    try:
        ...
        is_tied_game = (not game['is_active'] and 
                       game['winner'] is None and 
                       game['player1_score'] == game['player2_score'])
        
        tie_breaker_possible = (is_tied_game and 
                              len(p1_deck) > 0 and 
                              len(p2_deck) > 0)
        ...
        return jsonify({'error': f'Failed to check tie-breaker status: {str(e)}'}), 500
...
def tie_breaker_round(game_id):
    ...
    try:
        ...
        return jsonify({
            ...
            'final_winner': tie_breaker_winner if not tie_breaker_tied else None,
            'still_tied': tie_breaker_tied
        }), 200
        
    except Exception as e:
        return jsonify({'error': f'Failed to resolve tie-breaker: {str(e)}'}), 500
