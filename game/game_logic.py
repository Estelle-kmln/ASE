...
# GameRepository imported inside functions to avoid circular import
...
class Deck:
    ...
    def __init__(self, cards: List[Card]):
        ...
        if len(cards) != self.DECK_SIZE:
            raise ValueError(
                f"Deck must contain exactly {self.DECK_SIZE} cards, got {len(cards)}"
            )
...
class Hand:
    ...
    def __init__(self, cards: List[Card]):
        ...
        if len(cards) != self.HAND_SIZE:
            raise ValueError(
                f"Hand must contain exactly {self.HAND_SIZE} cards, got {len(cards)}"
            )
...
class BattleCardGame:
    ...
    def start_new_game(
        ...
        player1_name: str,
        player2_name: str,
        player1_deck: Optional[Deck] = None,
        player2_deck: Optional[Deck] = None,
    ) -> Game:
    ...
    def resolve_round(self) -> dict:
        ...
        if not player1_card or not player2_card:
            raise ValueError(
                "Both players must play cards before resolving round"
            )
        ...
        return {
            ...
            "game_winner": (
                ...
                if self.game.winner
                else ("Tie" if self.game.game_over else None)
            ),
...
def start_two_player_game():
    ...
    # Get player names
    player1_name = (
        input("\nEnter Player 1 name (default: Player 1): ").strip()
        or "Player 1"
    )
    player2_name = (
        input("Enter Player 2 name (default: Player 2): ").strip() or "Player 2"
    )
...
def play_game_loop(game: Game):
    ...
    while not game.game_over:
        ...
        print(f"{'='*60}")
        print(
            f"Score: {game.player1.name} {game.player1.score} - {game.player2.score} {game.player2.name}"
        )
        print(
            f"Cards remaining: {game.player1.name} {len(game.player1.deck)} - {len(game.player2.deck)} {game.player2.name}"
        )
        ...
        if round_result["game_over"]:
            ...
            print(
                f"Final Score: {game.player1.name} {round_result['player1_score']} - {round_result['player2_score']} {game.player2.name}"
            )
...
def get_player_card_choice(player: Player) -> Optional[int]:
    ...
    while True:
        ...
        choice = (
            input("Enter card number (1-3, or 'q' to quit): ").strip().lower()
        )
...
def show_card_statistics():
    ...
    for symbol, count in sorted(symbol_counts.items()):
        emoji = {"rock": "🪨", "paper": "📄", "scissors": "✂️"}.get(
            symbol, symbol
        )
