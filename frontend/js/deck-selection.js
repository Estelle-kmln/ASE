...
async function confirmDeck() {
  ...
  if (deckMode === "manual") {
    ...
    if (total !== DECK_SIZE) {
      alert(
        `Please select exactly ${DECK_SIZE} cards. Currently selected: ${total}`
      );
      return;
    }
  ...
  pollInterval = setInterval(async () => {
    ...
    try {
      ...
      if (response.status === 401 || response.status === 404) {
        ...
        clearInterval(pollInterval);
        alert(
          "The game invitation was cancelled or declined by your opponent."
        );
        window.location.href = "index.html";
        return;
      }
