...
function isGameOverState(state) {
  ...
  const statusOver =
    ...
    state.game_status === "abandoned" ||
    state.game_status === "ignored";
  const roundsOver =
    Array.isArray(state.round_history) && state.round_history.length >= 7;
  return statusOver || roundsOver;
}
...
function updateGameDisplay() {
  ...
  if (!myState.has_drawn) {
    ...
    viewDetailsButton.disabled =
      hand.length === 0 || selectedCardIndex === null;
  ...
  if (
    ...
    gameState.game_status === "abandoned" ||
    gameState.game_status === "ignored"
  ) {
    ...
    if (
      !gameState.winner &&
      !gameState.awaiting_tiebreaker_response &&
      !(
        gameState.player1_tiebreaker_decision ||
        gameState.player2_tiebreaker_decision
      )
    ) {
  ...
  if (
    gameState.player1_tiebreaker_decision === "yes" &&
    gameState.player2_tiebreaker_decision === "yes" &&
    gameState.game_status === "active" &&
    !gameState.player1_played_card &&
    !gameState.player2_played_card
  ) {
...
async function drawCards() {
  if (isGameOverState(gameState)) {
    alert("Game is finished. No further draws allowed.");
    await loadGameState();
    return;
  }
  const token = localStorage.getItem("token");
...
function viewCardDetails() {
  ...
  document.getElementById("detail-card-emoji").textContent =
    cardEmojis[selectedCard.type];
  ...
  const powerInfo = `\n\nPower Mechanics:\n• Higher power wins ties (e.g., Power ${
    selectedCard.power
  } beats Power ${selectedCard.power - 1})${
    selectedCard.power === 13 ? "\n• Power 13 is the strongest!" : ""
  }${
  ...
  document.getElementById("detail-card-strategy").textContent =
    baseStrategyInfo[selectedCard.type] + powerInfo;
  ...
  try {
    ...
    if (response.ok) {
      ...
      document.getElementById("turn-indicator").textContent =
        "Card played! Waiting for opponent...";
...
function updatePlayedCards() {
  ...
  if (myState.played_card) {
    ...
    const myLastCard = isPlayer1
      ? gameState.last_round.player1_card
      : gameState.last_round.player2_card;
    if (myLastCard) {
      myCard.innerHTML = `
  ...
  if (myState.played_card && opponentState.played_card) {
    ...
    const opponentLastCard = isPlayer1
      ? gameState.last_round.player2_card
      : gameState.last_round.player1_card;
    if (opponentLastCard) {
      opponentCard.innerHTML = `
...
async function submitTiebreakerDecision(decision) {
  ...
  try {
    const response = await fetch(
      `${GAME_API_URL}/${gameId}/tiebreaker-decision`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
function showTiebreakerPlayInterface() {
  ...
  const isPlayer1 = currentUser.username === gameState.player1.name;
  const myPlayedCard = isPlayer1
    ? gameState.player1.played_card
    : gameState.player2.played_card;

  if (myPlayedCard) {
    // Already played, waiting for opponent
    const indicator = document.getElementById("turn-indicator");
    indicator.textContent =
      "Your tiebreaker card has been played! Waiting for opponent...";
    indicator.style.color = "#7f8c8d";

    // Check if both played to resolve
    const opponentPlayedCard = isPlayer1
      ? gameState.player2.played_card
      : gameState.player1.played_card;
...
async function playTiebreakerCard() {
  ...
  const indicator = document.getElementById("turn-indicator");
  indicator.innerHTML =
    '<div style="text-align: center; color: #f39c12; font-size: 1.2rem;">⏳ Playing your tiebreaker card...</div>';
    ...
    if (response.ok) {
      ...
      indicator.innerHTML = `
                <div style="text-align: center; background: #27ae60; padding: 15px; border-radius: 8px; color: white;">
                    <p style="font-size: 1.3rem; margin: 0;">✨ You played: ${
                      cardEmojis[data.tiebreaker_card_played.type]
                    } Power ${data.tiebreaker_card_played.power} ✨</p>
                </div>
            `;
      ...
      alert(
        "Failed to play tiebreaker card: " + (data.error || "Unknown error")
      );
      await loadGameState();
    }
  } catch (error) {
