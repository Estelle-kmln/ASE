...
class CLI:
    ...
    def select_deck(self) -> List[Card]:
        ...
        print("\n=== Build Your Deck ===\n")
        print(f"Select {Deck.DECK_SIZE} cards for your deck from the collection.\n")
        ...
        while len(selected_cards) < Deck.DECK_SIZE:
            ...
            print(f"\nEnter card number (1-{len(all_cards)}) to add to deck (or 'q' to quit): ", end="")
            ...
            try:
                ...
                if not (0 <= card_index < len(all_cards)):
                    print(f"Invalid card number. Please enter a number between 1 and {len(all_cards)}.")
                    continue
                
                selected_card = all_cards[card_index]
                
                if selected_card in selected_cards:
                    print(f"Card {selected_card} is already in your deck. Please select a different card.")
                    continue
    ...
    def start_new_game(self):
        ...
        # Validate deck
        is_valid, error_msg = self.game_service.validate_deck_selection(selected_cards)
        if not is_valid:
            print(f"\nError: {error_msg}")
            return None
        ...
        # Start game
        try:
            game = self.game_service.start_new_game(deck)
            print(f"\n{'='*50}")
