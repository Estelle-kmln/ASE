...
resp = requests.post(f"{BASE_URL}/api/auth/register", json={"username": username, "password": "TestPass123"})
...
resp = requests.post(f"{BASE_URL}/api/auth/login", json={"username": username, "password": "WrongPassword"})
print(f"   Failed login: {resp.status_code}")

# 4. Successful login
print("\n4. Testing successful login...")
resp = requests.post(f"{BASE_URL}/api/auth/login", json={"username": username, "password": "TestPass123"})
token = resp.json()['access_token']
print(f"   Successful login: {resp.status_code}")

# 5. View profile
print("\n5. Testing profile view...")
resp = requests.get(f"{BASE_URL}/api/auth/profile", headers={"Authorization": f"Bearer {token}"})
print(f"   Profile viewed: {resp.status_code}")

# 6. Change password
print("\n6. Testing password change...")
resp = requests.put(f"{BASE_URL}/api/auth/profile", headers={"Authorization": f"Bearer {token}"}, json={"password": "NewTestPass123"})
...
resp = requests.post(f"{BASE_URL}/api/games", headers={"Authorization": f"Bearer {token}"}, json={"player2_name": "admin"})
if resp.status_code == 201:
    ...
    resp = requests.post(f"{BASE_URL}/api/games/{game_id}/cancel", headers={"Authorization": f"Bearer {token}"})
    print(f"   Game cancelled: {resp.status_code}")

# 10. Admin views logs
print("\n10. Testing admin log viewing...")
admin_resp = requests.post(f"{BASE_URL}/api/auth/login", json={"username": "admin", "password": "Admin123!"})
admin_token = admin_resp.json()['access_token']
resp = requests.get(f"{BASE_URL}/api/logs/list?page=0&size=20", headers={"Authorization": f"Bearer {admin_token}"})
print(f"   Admin viewed logs: {resp.status_code}")

# 11. Admin searches logs
print("\n11. Testing admin log search...")
resp = requests.get(f"{BASE_URL}/api/logs/search?query={username}", headers={"Authorization": f"Bearer {admin_token}"})
