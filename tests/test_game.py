...
def test_card_collection():
    ...
    # Verify all types and powers
    types = {"Rock", "Paper", "Scissors"}
    for card_type in types:
        cards = [c for c in collection.cards if c.type == card_type]
        assert (
            len(cards) == 13
        ), f"Expected 13 cards for {card_type}, got {len(cards)}"
    print("✓ All types have 13 cards (power 1-13)")
    print()
...
def test_random_deck():
    ...
    assert (
        len(deck1) == 22
    ), f"Random deck 1 should have 22 cards, got {len(deck1)}"
    assert (
        len(deck2) == 22
    ), f"Random deck 2 should have 22 cards, got {len(deck2)}"
...
def test_game_start():
    ...
    game = service.start_new_game(
        "Player1", deck1, "Player2", deck2, save=False
    )

    assert game.game_id is not None, "Game should have an ID"
    assert (
        game.turn_number == 1
    ), f"Game should start at turn 1, got {game.turn_number}"
    assert not game.game_over, "Game should not be over"
    assert (
        len(game.player1.deck) == 22
    ), f"Player1 deck should have 22 cards, got {len(game.player1.deck)}"
    assert (
        len(game.player2.deck) == 22
    ), f"Player2 deck should have 22 cards, got {len(game.player2.deck)}"
